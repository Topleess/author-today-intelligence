...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path=="/api/summary": return self._json({"works":db.execute("SELECT COUNT(*) FROM works").fetchone()[0],"snapshots":db.execute("SELECT COUNT(*) FROM work_snapshots").fetchone()[0],"archive captures":db.execute("SELECT COUNT(*) FROM archive_captures").fetchone()[0],"comments":db.execute("SELECT COUNT(*) FROM comments").fetchone()[0],"confirmed tags":db.execute("SELECT COUNT(*) FROM comment_tags WHERE confirmed=1").fetchone()[0],"hypotheses":"0 (not generated)"})
